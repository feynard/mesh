...
void Mesh::load(const char* obj_file) {
    // Clear previous data
    if (vertecis_ != 0) {
        delete[] vertecis_;
        delete[] normals_;
    ...
    if (normals_number_ != 0) {
        glBufferData(GL_ARRAY_BUFFER,
            faces_number_ * 11 * sizeof(vec3) + 24 * sizeof(vec3), faces_,
            GL_STATIC_DRAW);
        ...
        glBufferData(GL_ARRAY_BUFFER,
            faces_number_ * 3 * sizeof(vec3) + 24 * sizeof(vec3), faces_,
            GL_STATIC_DRAW);
        glBufferSubData(GL_ARRAY_BUFFER, faces_number_ * 3 * sizeof(vec3),
            24 * sizeof(vec3), bounding_box_);
    }
...
void Mesh::draw() {
    ...
    if (normals_number_ != 0 && fn_draw_ == true) {
        ...
        glPointSize(3);
        glDrawElements(GL_POINTS, faces_number_, GL_UNSIGNED_INT,
            faces_mid_indices_);
    }
    ...
    if (box_draw_) {
        ...
        if (normals_number_ != 0)
            ...
            glDrawArrays(GL_LINES, faces_number_ * 3, 24);
    }
}

void Mesh::set_color_attribute(GLuint color_location)
{
...
int Mesh::vertecis_number()
{
    return vertecis_number_;
}
...
int Mesh::faces_number()
{
    return faces_number_;
}
